...
def init_roles():
    """Initialize all CRM roles with custom incremental IDs and empty permissions."""
    repo = RoleRepository()
    ...
    roles = [
        ...
        {"name": "user", "description": "General user with limited access", "permissions": []}
    ]
    ...
    logging.info(f"Basic permissions initialized at [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]")
...
def init_permissions():
    ...
    role_permissions = {
        ...
        "manager": [
            "dashboard:read", "attendance:read", "attendance:update",
            "tasks:manage", "tasks:read", "tasks:create", "tasks:update", "tasks:delete", "users:read"
        ],
        "user": [
            "dashboard:read"
        ]
    ...
    for role_name, permissions_list in role_permissions.items():
        ...
        if role:
            ...
            logging.info(f"Updated permissions for role: {role_name}")
...
def create_admin_user():
    ...
    if not admin_password and not admin_email:
        logging.error("ADMIN EMAIL AND PASSWORD in env variable are not set. Cannot create admin user.")
        return
