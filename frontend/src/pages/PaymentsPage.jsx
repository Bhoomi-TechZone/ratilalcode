...
const PdfDownloadButton = ({ leadId }) => {
  const handleDownload = async () => {
    try {
      ...
      const res = await fetch(`https://ratilalandsons.onrender.com/api/quotations/${leadId}/invoice`, {
        method: "POST",
        headers: {
...
const PaymentsPage = () => {
  ...
  const fetchLeads = async () => {
    try {
      const res = await fetch('https://ratilalandsons.onrender.com/api/lead/leads?limit=1000', {
    ...
    try {
      const res = await fetch('https://ratilalandsons.onrender.com/api/quotations/', {
        headers: { "Authorization": `Bearer ${localStorage.getItem('access_token') || ''}` }
      });
      const data = await res.json();
...
const fetchData = async () => {
  ...
  const handleDuePaySubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('access_token') || '';
      const res = await fetch(`https://ratilalandsons.onrender.com/api/quotations/${duePayInvoice.lead_id}/duepay`, {
        method: "PATCH",
        headers: {
...
const handleNewInvoiceSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      setTimeout(() => {
        window.open(`https://ratilalandsons.onrender.com/api/quotations/${data.lead_id}/invoice`, "_blank");
      }, 1200);
    } catch (e) {
