...
const HRStaffModuleComplete = () => {
  ...
  const handleLeaveAction = async (requestId, action) => {
    ...
    try {
      ...
      const response = await fetch(`https://ratilalandsons.onrender.com/api/attendance/leave/admin/action/${requestId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          action: action,
          reviewed_by: currentUser.full_name || currentUser.name,
